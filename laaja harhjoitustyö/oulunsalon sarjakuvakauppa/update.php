...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_genre)){
        ...
        $author_err = "Please enter a valid genre.";
    }
     else{
        $genre = $input_genre;
    }
    ...
    if(empty($name_err) && empty($author_err) && empty($genre_err)){
        ...
        $sql = "UPDATE wantedlist SET book_n=:book_n, author=:author, genre=:genre WHERE book_id=:book_id";
    ...
    if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        ...
        $sql = "SELECT * FROM wantedlist WHERE book_id = :id";
        if($stmt = $pdo->prepare($sql)){
            // Bind variables to the prepared statement as parameters
